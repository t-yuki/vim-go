...
let s:test_handler_id = ''
let s:test_handler_jobs = {}

function! s:test_handler(job, exit_status, data)
    if !has_key(s:test_handler_jobs, a:job.id)
        return
    endif
    let l:compile = s:test_handler_jobs[a:job.id]
    if a:exit_status == 0
        if l:compile
    ...
    unlet s:test_handler_jobs[a:job.id]
endfunction
...
function! go#cmd#Test(bang, compile, ...)
    ...
    if has('nvim')
        ...
        if s:test_handler_id == ''
            let s:test_handler_id = go#jobcontrol#AddHandler(function('s:test_handler'))
        endif
        let s:test_handler_jobs[id] = a:compile
        return id
    endif
